...
class AuthController extends GetxController {
  ...
  static AuthController instance({String? tag}) =>
      Get.find<AuthController>(tag: tag);
  ...
  Future<void> logout() async {
    ...
    if (response?.statusCode == 200) {
      ...
      customSnackBar(response?.data['message']?.toString() ?? 'Error');
    }
    update();
  }
  ...
  bool hasAvatar() {
    final String avatar = userResponse?.user.avatar ?? '';
